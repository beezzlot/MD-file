...
Например, чтобы в 21:00 все компьютеры в офисе отключались или чтобы в 22:00 сервер автоматически отправлял журнал отчета о работе за весь рабочий день - и многое много другое, что хотелось бы автоматизировать, в-первую очередь для удобства, а во-вторую - лень же двигатель прогресса!

В этот момент, стоит поговорить о инструментах настройки выполнения различных задач по времени - такие как **systemd** и **cron**, а также о **incron** — который следит за событиями в файловой системе с помощью inotify и выполняет команду при наступлении указанного в задании события.
...
Теперь, как вы можете заметить значение Data изменилось - это связано с тем
